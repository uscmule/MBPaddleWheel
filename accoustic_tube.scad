...
module tube(){
    ...
    difference(){
        ...
        union(){
            ...
            // this rotation is easier to print
            translate([0,0,height + shaft_length*2])
            rotate([180,0,0])
            cage();
...
module cage() {
    ...
    for ( i = [0 : n] ){
        ...
        translate([0,0,z])
        spring(R=R, H=H, windings=windings, w = w,  h=h, pitch = pitch, start=true, end=true, center=false, reverse=rev);
    }
}

module port_tool(height, r1, r2, top_offset, side_offset) {
    port_tool_width = r1*2.5; // more than the diameter
    port_width = r2+2*side_offset;//r2-2*side_offset;
    port_height = height/3 +.1;//-2
    z = top_offset;
    z2 = top_offset + height/3;
    z3 = top_offset + (height/3 * 2);
    
    rotate([0,0,0])
    translate([-(port_width)/2,-port_tool_width/*/2*/,z])
    ...
    rotate([0,0,0])
    translate([-(port_width)/2,-port_tool_width/*/2*/,z2])
    cube([port_width,port_tool_width,port_height]);
    ...
    rotate([0,0,0])
    translate([-(port_width)/2,-port_tool_width/*/2*/,z3])
    cube([port_width,port_tool_width,port_height]);
